...
/**
 * Search for a movie by title and filter persons by their roles (Director, Actor, All)
 * @param {string} movieTitle - The title of the movie.
 * @param {string} personRole - The role of the person (Director, Actor, All).
 * @returns {Array} An array of person objects that match the criteria.
 */
Movie.searchByMovieAndRole = function(movieTitle, personRole) {
  let foundMovies = [];
  let foundPersons = [];

  // Find movies by title
  for (let id in this.instances) {
    if (this.instances[id].title.toLowerCase() === movieTitle.toLowerCase()) {
      foundMovies.push(this.instances[id]);
    }
  }

  // Filter persons by their roles
  for (let movie of foundMovies) {
    if (personRole.toLowerCase() === 'director') {
      foundPersons.push(movie.director);
    } else if (personRole.toLowerCase() === 'actor') {
      foundPersons = foundPersons.concat(movie.actors);
    } else if (personRole.toLowerCase() === 'all') {
      foundPersons.push(movie.director);
      foundPersons = foundPersons.concat(movie.actors);
    }
  }
...
Movie.generateTestData = function () {
  try {
    Movie.instances["1"] = new Movie({
      ...
      rating: 4,
      genre: [4, 13],
      director: new Person("3", "Quentin Tarantino"),
      actors: [
        new Person("5", "Uma Thurman"),
        new Person("6", "John Travolta")
      ]
    });
    Movie.instances["2"] = new Movie({
      ...
      rating: 2,
      genre: [12, 17, 15, 16],
      director: new Person("2", "George Lucas"),
      actors: [
        new Person("7", "Ewan McGregor"),
        new Person("8", "Natalie Portman")
      ]
    });
    Movie.instances["3"] = new Movie({
      ...
      rating: 5,
      genre: [4, 16],
      director: new Person("9", "The Wachowskis"),
      actors: [
        new Person("10", "Keanu Reeves"),
        new Person("11", "Laurence Fishburne"),
        new Person("12", "Carrie-Anne Moss")
      ]
    });
    Movie.instances["4"] = new Movie({
      ...
      rating: 5,
      genre: [4, 13],
      director: new Person("13", "Christopher Nolan"),
      actors: [
        new Person("14", "Leonardo DiCaprio"),
        new Person("15", "Joseph Gordon-Levitt"),
        new Person("16", "Ellen Page")
      ]
    });
    Movie.saveAll();
  } catch (e) {
